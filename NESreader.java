 ...
 * 			1					1 = Battery-backed PGR RAM goes at $6000-$7FFF or other persistant memory
	...
	public void readFile(CPU nes)
	{
		try 
		{
			...
			else
			{
				...
				ignoreMirroring = Boolean.parseBoolean(Integer.toString(header[6]&0x08));
				trainer = Boolean.parseBoolean(Integer.toString(header[6]&0x04));
				batteryBacked = Boolean.parseBoolean(Integer.toString(header[6]&0x02));
				mirroring = header[6]&0x01;
				if( (header[7]&0x0c) == 2)
				{
					nes2 = true;
				}
				playChoice = Boolean.parseBoolean(Integer.toString(header[7]&0x02));
				vsUni = Boolean.parseBoolean(Integer.toString(header[7]&0x01));
				prgRAM = header[8];
				...
				{
					nes.CPUmemory[0x8000+a] = (byte) fileIn.read();
				}
				
				for(int a = 0; a < (0x2000 * chrROMblocks); a++) //as is this is assuming no more than 1 block
				{
					nes.PPUmemory[a] = (byte) fileIn.read();
				}
			}
